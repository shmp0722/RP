function files = mrtrix_init_Tama2(dt6, lmax, mrtrix_folder, wmMaskFile)
...
[~,b,~]           = fileparts(dwrawfile);
a = dt_info.files.b0(1:end-13);
dwRawFile         = [a, b(1:end-4)];
...
[~,b,c] = fileparts(dt_info.files.alignedDwBvecs);
bv       = dlmread([a(1:end-8),b,c]);
...
%     brainMaskFile = fullfile(session, [brainmask, ext]);
